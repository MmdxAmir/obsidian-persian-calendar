...
## <a name="dynamic_path"></a> مسیردهی و نام پویا

می‌توانید مسیرهای یا نام یادداشت تقویم را به صورت پویا تعیین کنید.
...
| `jYY`         | 04          | سال شمسی دورقمی              |
| `YYYY`        | 2025        | سال میلادی چهاررقمی          |
| `YY`          | 25          | سال میلادی دورقمی            |
| `jQQQQ`       | Autumn      | نام کامل فصل شمسی            |
| `jQQ`         | 03          | شمارهٔ فصل شمسی دورقمی       |
| `jQ`          | 3           | شمارهٔ فصل شمسی              |
| `QQQQ`        | Autumn      | نام کامل فصل میلادی           |
| `QQQ`         | Aut         | نام سه‌حرفی فصل میلادی        |
| `QQ`          | 03          | شمارهٔ فصل میلادی دورقمی      |
| `Q`           | 3           | شمارهٔ فصل میلادی             |
| `jMMMM`       | Azar        | نام کامل ماه شمسی            |
| `jMMM`        | Aza         | نام سه‌حرفی ماه شمسی         |
| `jMM`         | 09          | شمارهٔ ماه شمسی دورقمی       |
| `jM`          | 9           | شمارهٔ ماه شمسی              |
| `MMMM`        | November    | نام کامل ماه میلادی          |
| `MMM`         | Nov         | نام سه‌حرفی ماه میلادی       |
| `MM`          | 11          | شمارهٔ ماه میلادی دورقمی     |
| `M`           | 11          | شمارهٔ ماه میلادی            |
| `jDD`         | 05          | شمارهٔ روز ماه شمسی دورقمی   |
| `jD`          | 5           | شمارهٔ روز ماه شمسی          |
| `DDDD`        | دوشنبه      | روز هفته                     |
| `DD`          | 06          | شمارهٔ روز ماه میلادی دورقمی |
| `D`           | 6           | شمارهٔ روز ماه میلادی        |
| `ww`          | 49          | شمارهٔ هفته دورقمی            |
| `w`           | 49          | شمارهٔ هفته                  |

</div>

عبارت‌های `w` و `ww` برای درج شمارهٔ هفته در مسیر یا نام یادداشت استفاده می‌شوند. مقدار آن‌ها بر اساس
نحوه‌ی محاسبه‌ی هفته‌نوشت در تنظیمات افزونه تعیین می‌شود؛ یعنی می‌توانند از شماره‌گذاری هفتهٔ شمسی
یا میلادی و قوانین شروع هفتهٔ انتخاب‌شده پیروی کنند.

این دو عبارت برای مسیر هفته‌نوشت کاربرد ویژه‌ای دارند و می‌توان از آن‌ها در مسیر روزنوشت نیز استفاده
کرد تا روزنوشت در پوشه یا مسیر هفتهٔ متناظر قرار بگیرد.

اگر در مسیر هفته‌نوشت، در کنار `w` یا `ww` از متغیرهای تاریخ در بخش‌های بالادستی مسیر استفاده شود،
می‌توان تعیین کرد که این متغیرها بر اساس **ابتدای هفته** یا **انتهای هفته** محاسبه شوند. این گزینه با
عنوان **مبنای تاریخ مسیر هفته‌نوشت** در تنظیمات در دسترس است و مقدار پیش‌فرض آن **ابتدای هفته** است.

برای هفته‌ای که با محاسبه‌ی **اولین هفتهٔ کامل سال** از مرز سال عبور می‌کند، گزینه‌ی جداگانه‌ی
**مبنای هفتهٔ مرزی سال** تعیین می‌کند که تاریخ‌های مسیر از ابتدای آن هفته یا انتهای آن هفته گرفته شوند.
این گزینه فقط برای همان هفتهٔ مرزی اعمال می‌شود و سایر هفته‌ها همچنان از مبنای عادی مسیر هفته‌نوشت
استفاده می‌کنند.

برای مثال در مسیری مانند `jYYYY/jQQQQ/jMM/[W]ww`، سال، فصل و ماه بر اساس مبنای انتخاب‌شده برای هفته
محاسبه می‌شوند، در حالی که `[W]` به‌صورت یک نام ثابت باقی می‌ماند. بنابراین برای یک هفتهٔ مرزی سال
می‌توان مسیر را به‌صورت سازگار زیر سالی قرار داد که هفته از آن شروع می‌شود یا سالی که هفته در آن پایان
می‌یابد.

> **نکته:** روزنوشت همیشه از تاریخ واقعی همان روز استفاده می‌کند. هنگام انتخاب **اولین هفتهٔ کامل سال**،
> استفاده از `w` یا `ww` در مسیر روزنوشت توصیه نمی‌شود، زیرا ممکن است یک هفته بین دو سال قرار بگیرد.

براکت‌ها به تابع مسیردهی می‌گویند که محتوای داخل آن باید به‌عنوان یک نام ثابت در نظر گرفته شود:

- عنوان Daily => به‌صورت D + aily تفسیر می‌شود.
- عنوان [Daily] => به‌صورت نام ثابت Daily تفسیر می‌شود.
...
// مناسبت‌ها\ npcApi.checkHoliday(new Date()); // آیا روز تعطیل است؟ true/false
...
- امکان تعیین نحوه‌ی محاسبه‌ی هفته‌نوشت روی میلادی یا شمسی
- برای سازگاری این پلاگین با دیگر پلاگین‌های تقویم میلادی می‌توانید ماه‌نوشت و سال‌نوشت شمسی را به صورت میلادی ثبت کنید.
