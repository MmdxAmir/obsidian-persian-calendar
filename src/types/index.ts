...
export type TJalali = { jy: number; jm: number; jd: number };
// gregorian = میلادی
export type TGregorian = { gy: number; gm: number; gd: number };
// hijri = هجری قمری
export type THijri = { hy: number; hm: number; hd: number };
...
export type TGetGregorianDayOfWeek = { gYear: number; gWeekNumber: number };
...
export type TBuildContext = { currentDate: Date; fileDate: Date | null; fileName: string; baseDate: TDateFormat };
export type TMonthGridCell = TJalali & { date: Date; gregorian: TGregorian; hijri: THijri; index: number; row: number; column: number; isInCurrentMonth: boolean; isToday: boolean; isWeekend: boolean; isHolidayInIran: boolean };
export type TSuggestProvider = { trigger: RegExp; getSuggestions: (query: string) => string[]; onSelect: (value: string, ctx: EditorSuggestContext) => string };
...
export type TCompiledPattern = { pattern: string; locale: TLocale; segments: TPatternSegment[]; regex: RegExp; fields: TTokenField[] };
export type TValidationSeverity = "error" | "warning";
export type TValidationError = { type: "empty-pattern" | "unknown-token" | "ambiguous-adjacent-numeric" | "duplicate-field"; severity: TValidationSeverity; message: string; token?: string };
export type TValidationResult = { valid: boolean; errors: TValidationError[] };
export type TWeekCalculator = { getWeekNumber(date: Date, weekStart?: TWeekStart): { jy: number; weekNumber: number }; getStartOfWeek(jYear: number, jWeekNumber: number, weekStart?: TWeekStart): TJalali & TGregorian; getEndOfWeek(jYear: number, jWeekNumber: number, weekStart?: TWeekStart): TJalali & TGregorian };
export type TToken = "YYYY" | "YY" | "jYYYY" | "jYY" | "QQQQ" | "QQQ" | "QQ" | "Q" | "jQQQQ" | "jQQ" | "jQ" | "MMMM" | "MMM" | "MM" | "M" | "jMMMM" | "jMMM" | "jMM" | "jM" | "DDDD" | "DD" | "D" | "jDD" | "jD" | "ww" | "w";
