...
| `jalaliToStartWeek`            | `(jy: number)`                         | `{jy, jm, jd}`                                  | Returns the first day of the first week of a Jalali year (first Saturday of the year)                 |
...
## Weekly Path Date Anchoring

Weekly paths can use week tokens together with date-based tokens. When a weekly path contains both a week token and at least one date-based token, the date-based tokens are resolved from either the start or the end of the week according to the selected weekly path date anchor.

For example, a weekly path such as `jYYYY/jMM/ww` can use the start of the week to resolve the year and month, or the end of the week when the path is configured accordingly. This is important for weeks that cross month or year boundaries.

The anchor is only relevant when the weekly path contains both a week token and a date-based token. A path containing only a week token, such as `ww`, does not require an anchor.

Gregorian quarter tokens are also supported in weekly paths:

| Token   | Output | Description |
| ------- | ------ | ----------- |
| `Q`     | `1`–`4` | Numeric Gregorian quarter |
| `QQ`    | `01`–`04` | Two-digit Gregorian quarter |
| `QQQ`   | `Spr` / `Sum` / `Aut` / `Win` | Short Gregorian quarter name |
| `QQQQ`  | `Spring` / `Summer` / `Autumn` / `Winter` | Full Gregorian quarter name |

When a weekly path crosses a month or year boundary, the selected anchor determines which side of the week supplies the date values. For example, if the start of a week is in one Gregorian quarter and the end is in the next, `Q`-based values can differ depending on whether the anchor is set to the start or end of the week.
