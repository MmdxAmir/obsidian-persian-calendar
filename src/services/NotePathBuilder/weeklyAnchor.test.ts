...
const makePlugin = (
	...
	setting: { weeklyNotesPath, weeklyPathAnchor, weekCalculation } as Pick<TSetting, "weeklyNotesPath" | "weeklyPathAnchor" | "weekCalculation">,
} as never);

describe("Weekly path anchor visibility", () => {
	const dateTokens = ["YYYY", "YY", "MMMM", "MMM", "MM", "M", "DD", "D", "jYYYY", "jYY", "jMMMM", "jMMM", "jMM", "jM", "jDD", "jD", "Q", "QQ", "QQQ", "QQQQ", "jQ", "jQQ", "jQQQQ"];
...
describe("Weekly path anchor resolution", () => {
	...
	it("uses the Gregorian week calculator", () => {
		...
		const start = new NotePathBuilder(makePlugin("YYYY/QQQQ/ww", "start", "gregorian-first-day-of-year")).buildWeeklyNotePath(weekYear, weekNumber);
		const end = new NotePathBuilder(makePlugin("YYYY/QQQQ/ww", "end", "gregorian-first-day-of-year")).buildWeeklyNotePath(weekYear, weekNumber);
		expect(start.filePath).toContain("2026/Spring/");
		expect(end.filePath).toContain("2026/Summer/");
	});
});
